/**
 * SettingsGA.tsx — Plugin Google Analytics
 *
 * UI para configurar o Google Analytics Measurement ID.
...
export default function SettingsGA() {
    ...
    useEffect(() => {
        githubApi('read', CONFIG_PATH)
            .then(data => {
                ...
                setMeasurementId(config?.googleAnalytics?.measurementId || '');
                setFileSha(data.sha);
            })
    ...
    const handleSave = async () => {
        ...
        setSaved(false);
        setError('');
        triggerToast('Salvando configuração do Google Analytics...', 'progress', 30);
        try {
            const updated = {
                ...fullConfig,
                googleAnalytics: { measurementId: measurementId.trim() },
            };
            const res = await githubApi('write', CONFIG_PATH, {
                content: JSON.stringify(updated, null, 4),
                sha: fileSha,
                message: 'CMS: Update Google Analytics measurement ID',
            });
            setFileSha(res.sha || fileSha);
            setFullConfig(updated);
            setSaved(true);
            triggerToast('Google Analytics configurado!', 'success', 100);
    ...
    return (
        <div className="max-w-2xl space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                <h3 className="font-bold text-slate-800 mb-1">Measurement ID</h3>
                <p className="text-sm text-slate-500 mb-4">
                    Encontre o Measurement ID no painel do Google Analytics em{' '}
                    <span className="font-mono text-violet-600">Admin → Data Streams → seu site</span>.
                    O formato é <span className="font-mono font-bold">G-XXXXXXXXXX</span>.
                </p>
                ...
                {measurementId && !measurementId.startsWith('G-') && (
                    <p className="text-xs text-amber-600 mt-2 ml-1">
                        O Measurement ID geralmente começa com "G-". Verifique se está correto.
                    </p>
                )}
            </div>
            ...
            <div className="bg-slate-50 rounded-2xl border border-slate-200 p-4">
                ...
                {measurementId && (
                    <div className="flex items-center justify-between text-sm mt-2">
                        ...
                        <span className="font-mono font-bold text-slate-700">{measurementId}</span>
                    </div>
                )}
            </div>
            ...
            <div className="bg-blue-50 rounded-2xl border border-blue-200 p-5">
                ...
                <ol className="space-y-2">
                    {[
                        'Acesse analytics.google.com e crie uma propriedade GA4',
                        'Em Admin → Data Streams, clique no seu stream web',
                        'Copie o Measurement ID (formato G-XXXXXXXXXX)',
                        'Cole aqui e clique em "Salvar Configuração"',
                        'O script será inserido automaticamente em todas as páginas',
                    ].map((step, i) => (
